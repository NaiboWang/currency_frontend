<template>
  ...
  <el-card>
    <h2>Welcome to Currency Market!</h2>
  ...
  <el-dialog title="Advanced Search" v-model="dialogFormVisible">
    <el-form>
      <el-form-item v-for="queryField in queryFields" :label="queryField.label+queryField.comment"
                    :key="queryField.label"
                    prop="queryField.value">

        <el-input v-model="queryInfo.multiConditions[queryField.value]" v-if="queryField.type=='text'"
                  autocomplete="off"></el-input>
        <!--number-->
        <div v-if="queryField.type=='number'" style="width: 100%">
          <el-input v-model="queryInfo.multiConditions[queryField.value +'_from']" style="width: 49%" placeholder="From"
                    type="number" autocomplete="off"></el-input>
          <el-input v-model="queryInfo.multiConditions[queryField.value +'_to']" style="width: 49%; float: right"
                    placeholder="To"
                    type="number" autocomplete="off"></el-input>
        </div>
        ...
        <div v-if="queryField.type=='datetime'" style="width: 100%">
          <el-input v-model="queryInfo.multiConditions[queryField.value +'_from']" style="width: 47%" placeholder="From"
                    type="datetime-local" autocomplete="off"></el-input>
          <label style="width: 4%;margin-left: 2%">To</label>
          <el-input v-model="queryInfo.multiConditions[queryField.value +'_to']" style="width: 47%; float: right"
                    placeholder="To"
                    type="datetime-local" autocomplete="off"></el-input>
        </div>
    ...
    initSearch: function () {
      for (let item of this.queryFields) {
        if (item.type == 'number' || item.type == 'datetime') {
          this.queryInfo.multiConditions[item.value+'_from'] = '';
          this.queryInfo.multiConditions[item.value+'_to'] = '';
        } else {
